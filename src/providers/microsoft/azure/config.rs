...
fn load_crypt_r() -> Result<CryptRFn> {
    ...
    match CACHED.get_or_init(|| {
        ...
        for soname in CANDIDATES {
            // SAFETY: `soname` is a valid NUL-terminated C string; the null
            ...
            // SAFETY: `handle` is non-null (checked above) and the symbol name
            // is a valid C string; the null return is checked below.
            let sym = unsafe { libc::dlsym(handle, c"crypt_r".as_ptr()) };
            if sym.is_null() {
                last_err = format!("{} did not export crypt_r", soname.to_string_lossy());
                continue;
            }
            // SAFETY: `sym` is non-null (checked above) and points to `crypt_r`,
            // whose C ABI matches `CryptRFn`.
...
fn sha512_crypt(password: &str, salt: &str, rounds: usize) -> Result<String> {
    ...
    // SAFETY: CryptData is a `repr(C)` struct of integer/char arrays, so an
    // all-zero bit pattern is a valid, initialized value.
    let mut data: Box<CryptData> = unsafe { Box::new(std::mem::zeroed()) };

    // SAFETY: `phrase` and `setting` are valid C strings and `data` is a valid,
    // zeroed CryptData; all outlive the call. The null return is checked below.
    let out_ptr = unsafe { crypt_r(phrase.as_ptr(), setting.as_ptr(), &mut *data) };
    if out_ptr.is_null() {
        anyhow::bail!("crypt_r failed: {}", std::io::Error::last_os_error());
    }
    // SAFETY: `out_ptr` is non-null (checked above) and points to a
    // NUL-terminated string in `data`, which outlives this borrow.
