...
impl CliRenderIgnition {
    ...
    pub(crate) fn run(self) -> Result<()> {
        ...
        if hostname {
            metadata
                .write_hostname_ignition_fragment(&self.render_ignition_dir)
                .context("generating hostname ignition fragment")?;
        }

        if platform_user {
            metadata
                .write_user_ignition_fragment(&self.render_ignition_dir)
...
mod tests {
    ...
    impl MetadataProvider for FakeProvider {
        ...
        fn ssh_keys(&self) -> Result<Vec<PublicKey>> {
            self.ssh_keys
                ...
                .map(|s| {
                    s.parse::<PublicKey>()
                        .context("failed to parse test ssh key")
                })
                .collect()
        }
    ...
    #[test]
    fn test_write_hostname_ignition_fragment_writes_file() {
        ...
        provider.write_hostname_ignition_fragment(dir).unwrap();
    ...
    #[test]
    fn test_write_hostname_ignition_fragment_skipped_when_absent() {
        let tmp = tempfile::tempdir().unwrap();
        let dir = tmp.path().to_str().unwrap();
    ...
    #[test]
    fn test_write_user_ignition_fragment_includes_password_hash() {
        let tmp = tempfile::tempdir().unwrap();
        let dir = tmp.path().to_str().unwrap();
        ...
        provider.write_user_ignition_fragment(dir).unwrap();
    ...
    #[test]
    fn test_write_user_ignition_fragment_skipped_without_username() {
        let tmp = tempfile::tempdir().unwrap();
        let dir = tmp.path().to_str().unwrap();
    ...
    #[test]
    fn test_write_user_ignition_fragment_skipped_without_credentials() {
        let tmp = tempfile::tempdir().unwrap();
        let dir = tmp.path().to_str().unwrap();
