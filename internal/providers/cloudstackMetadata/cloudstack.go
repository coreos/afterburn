...
func FetchMetadata() (providers.Metadata, error) {
	...
	if !fileExists(configDriveMetadataMountPoint) {
		err := error(nil)
		mnt, err = mountConfigDrive("config");
		if err != nil {
			if mnt, err = mountConfigDrive("CONFIG"); err != nil {
				return m, err
			}
		}
...
func getPath(label string) (string, error) {
	...
	out, err := exec.Command("blkid", "-l", "-t", "LABEL="+label, "-o", "device").Output()
	if err != nil {
		return "", err
	}
...
func readFromConfigDrive(mnt string, file string) (string, bool, error) {
	if !fileExists(filepath.Join(mnt, configDriveMetadataPath, file)) {
		return "", false, nil
	}

	output, err := ioutil.ReadFile(filepath.Join(mnt, configDriveMetadataPath, file))
